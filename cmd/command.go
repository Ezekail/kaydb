...
var (
	errSyntax            = errors.New("ERR syntax error 语法错误")
...
func newWrongNumOfArgsError(cmd string) error {
	return fmt.Errorf("ERR wrong num of arguments for '%s' command", cmd)
}
...
func strScan(cli *Client, args [][]byte) (interface{}, error) {
	if len(args) != 3 {
		return nil, newWrongNumOfArgsError("scan")
	}
	...
	return cli.db.StrScan(args[0], pattern, count)

}

// Todo
func scan(cli *Client, args [][]byte) (interface{}, error) {
	if len(args) > 5 || len(args) <= 0 {
		return nil, newWrongNumOfArgsError("scan")
	}
	cur, err := strconv.Atoi(string(args[0]))
	if err != nil {
		return nil, err
	}
	...
	_, val, err := cli.db.Scan(cur, pattern, count)
	if err != nil {
		return nil, err
	}
	return val, nil
...
func getRange(cli *Client, args [][]byte) (interface{}, error) {
	if len(args) != 3 {
	...
	return cli.db.GetRange(key, start, end)
}
...
func lPushX(cli *Client, args [][]byte) (interface{}, error) {
	if len(args) < 2 {
		return nil, newWrongNumOfArgsError("lpushx")
	}
	key, value := args[0], args[1:]
...
func rPushX(cli *Client, args [][]byte) (interface{}, error) {
	if len(args) < 2 {
		return nil, newWrongNumOfArgsError("rpushx")
	}
	key, value := args[0], args[1:]
...
func popInternal(db *kaydb.KayDB, args [][]byte, isLeft bool) (interface{}, error) {
	if len(args) < 1 {
		return nil, newWrongNumOfArgsError("pop")
	}
	key := args[0]
	// 输入参数有pop的数量
	var count = 1
...
func lMove(cli *Client, args [][]byte) (interface{}, error) {
	...
	// 获取位置方向的参数
	form, to := strings.ToLower(string(args[2])), strings.ToLower(string(args[3]))
...
func lIndex(cli *Client, args [][]byte) (interface{}, error) {
	...
	return cli.db.LIndex(key, indexInt)
}
...
func lSet(cli *Client, args [][]byte) (interface{}, error) {
	...
	err = cli.db.LSet(key, value, indexInt)
	if err != nil {
		return nil, err
	}
	return redcon.SimpleString(resultOK), nil
...
func lRange(cli *Client, args [][]byte) (interface{}, error) {
	...
	return cli.db.LRange(key, startInt, endInt)
}
...
func hIncrBy(cli *Client, args [][]byte) (interface{}, error) {
	...
	incrInt64, err := util.StrToInt64(string(incrVal))
	if err != nil {
		return nil, errValueIsInvalid
	}
	return cli.db.HIncrBy(key, field, incrInt64)
}
...
func sAdd(cli *Client, args [][]byte) (interface{}, error) {
	...
	for _, v := range args[1:] {
		isMember := cli.db.SIsMember(key, v)
		if !isMember {
			err := cli.db.SAdd(key, v)
			if err != nil {
				return nil, err
			}
			count++
...
func sPop(cli *Client, args [][]byte) (interface{}, error) {
	...
	return cli.db.SPop(key, uint(count))
}
...
func sIsMember(cli *Client, args [][]byte) (interface{}, error) {
	...
	res := make([]redcon.SimpleInt, 0, len(args[1:]))
	key := args[0]
...
func zRange(cli *Client, args [][]byte) (interface{}, error) {
	...
	return cli.db.ZRange(args[0], start, end)
}
...
func zRevRange(cli *Client, args [][]byte) (interface{}, error) {
	...
	return cli.db.ZRevRange(args[0], start, end)
}
